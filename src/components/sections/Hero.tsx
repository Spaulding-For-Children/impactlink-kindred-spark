...
export const Hero = () => {
  ...
  const actionCards = [
    ...
    { icon: Users, title: t("hero.connectAgencies"), description: t("hero.connectAgenciesDesc"), color: "from-sage to-sage/80", href: "/agencies" },
    { icon: Database, title: t("hero.accessData"), description: t("hero.accessDataDesc"), color: "from-amber to-amber-light", href: "/data-tools" },
    { icon: Lightbulb, title: t("hero.submitIdea"), description: t("hero.submitIdeaDesc"), color: "from-navy-light to-navy", href: "/collaboration" },
  ];
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
          {actionCards.map((card, index) => (
            <motion.div key={card.title} onClick={() => navigate(card.href)} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 + index * 0.1 }} whileHover={{ y: -5, scale: 1.02 }} className="group relative p-6 rounded-2xl bg-card border border-border shadow-soft hover:shadow-elevated transition-all duration-300 overflow-hidden cursor-pointer">
