...
export default function Directory() {
  const { profiles: allProfiles, loading, counts } = useProfiles();
  ...
  const allTags = useMemo(() => 
    [...new Set(allProfiles.flatMap((p) => p.tags))].sort(), 
    [allProfiles]
  );
  const allLocations = useMemo(() => 
    [...new Set(allProfiles.map((p) => p.location).filter(Boolean))].sort(), 
    [allProfiles]
  );
  ...
  const filteredProfiles = useMemo(() => {
    ...
    return result;
  }, [allProfiles, searchQuery, profileType, selectedTags, selectedLocations, sortBy]);
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-8 text-center">
                <div>
                  <p className="text-3xl font-display font-bold text-amber">{counts.students}</p>
                  <p className="text-sm text-muted-foreground">Students</p>
                </div>
                <div>
                  <p className="text-3xl font-display font-bold text-navy">{counts.researchers}</p>
                  <p className="text-sm text-muted-foreground">Researchers</p>
                </div>
                <div>
                  <p className="text-3xl font-display font-bold text-sage">{counts.agencies}</p>
